  ...
  def run(repo, candidate_ref, %Goal{} = verifier, targets, fault_patch, opts \\ []) do
    ...
    try do
      with {:ok, sha} <-
           ...
           {:ok, _} <- git(repo, ["worktree", "add", "--detach", work, String.trim(sha)], timeout) do
        # Freeze checker inputs at the candidate before applying the fault.
        manifest = Seal.arm(verifier.seal, nil, work)

        with {:ok, baseline} <- check(verifier, work, timeout),
             %{eligible: _} <-
             ...
             {:ok, mutated} <- check(%{verifier | setup: nil}, work, timeout),
    ...
    after
      git(repo, ["worktree", "remove", "--force", work], timeout)
      File.rm_rf!(root)
    end
  end

  defp check(goal, work, timeout) do
    task =
      Task.async(fn ->
        try do
