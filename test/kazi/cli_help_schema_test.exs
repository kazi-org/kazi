defmodule Kazi.CLIHelpSchemaTest do
  ...
  describe "run/2 — help --json" do
    ...
    test "lists every command the parser dispatches (stays in sync with the real table)" do
      ...
      expected =
        MapSet.new(
          ~w(apply status orphans init install-skill install-hooks mcp dashboard daemon bus economy plan list-proposed approve reject export lint spec context memory help schema version)
        )
  ...
  defp dispatch_probe(name) do
    argv =
      case name do
        ...
        # `daemon` requires a <subcommand> positional; probe with a real one
