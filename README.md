...
- **Scope roots as a directory prompt** — a goal's `[scope].write_paths` (or `paths`) doubles as a rendered `AGENTS.md`/`CLAUDE.md` node at the scope root, so `cd`-ing into a goal's directory and opening a harness by hand gets the same brief and failing predicates a dispatched agent gets, through the harness's own walk-up convention (`kazi plan render --tree`, `kazi apply --cwd`; see [`AUTHORING.md`](docs/AUTHORING.md#scope-roots-and-the-agentsmd-node) and [ADR-0086](docs/adr/0086-write-paths-and-the-agents-md-projection.md)).
