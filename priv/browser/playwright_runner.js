...
async function evalAssertion(page, assertion, timeout) {
  ...
  switch (assertion.type) {
    case "visible": {
      const visible = await page
        ...
        .then(() => true)
        .catch(() => false);
      return { ...base, ok: visible, expected: "visible", found: visible ? "visible" : "not visible" };
    }
    case "hidden": {
      const hidden = await page
        ...
        .then(() => true)
        .catch(() => false);
      return { ...base, ok: hidden, expected: "hidden", found: hidden ? "hidden" : "visible" };
    }
    case "text": {
      const text = await page
        .textContent(assertion.selector, { timeout })
        .catch(() => null);
    ...
    case "url": {
      ...
      return { ...base, ok, expected: assertion.exact ?? assertion.contains, found: current };
    }
    default:
...
async function main() {
  ...
  try {
    ...
    for (const assertion of payload.assertions || []) {
      assertions.push(await evalAssertion(page, assertion, timeout));
    }
