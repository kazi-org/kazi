...
| `trailer`        | string | A task-identifying trailer VALUE only (TKE.4). `"Plan-row: <id>"` when `--lane-contract`/`KAZI_LANE_CONTRACT` names a contract carrying a non-empty `"task"` field (a sire-style task id, TKE.1) -- reusing the literal `Plan-row:` key sire's own Intake gate already checks for, so a kazi-driven sire lane keeps passing sire's existing `check-row-trailer.sh`-style gate unmodified. Falls back to `"Kazi-Goal: <goal-id>"` when no `--lane-contract` was given, the file is unreadable/unparseable, or it carries no `"task"` field (the generic hq session-container case). kazi computes the value only; stamping it onto a commit/PR is the hook's own git-level mechanics (decided design 3.2) -- kazi itself makes zero `git commit`/`git config` calls in lane mode. |
