...
| `reason` | string \| null | Disambiguates `stuck` (the loop's same failing set persisted across N observations within the run, T1.5) from a budget dimension (`max_iterations` / `wall_clock` / `token_budget`). |
