...
- [ ] T27.7 Deprecation policy note -- removal version DECIDED (operator 2026-06-24): `run`/`propose`/`mix kazi.run` are deprecated aliases removed in **v0.5.0** (the next minor). Write a short `docs/` note (+ CHANGELOG entry) stating the aliases, the rationale (verb unification, ADR-0032), and the v0.5.0 removal.  Owner: TBD  Est: 0.5h  verifies: [UC-041]  delivers: [a documented v0.5.0 deprecation window for run/propose]  deps: [T27.1]  acc: the note names the aliases + the ADR-0032 rationale + the concrete v0.5.0 removal; linked from the CHANGELOG; the deprecation hint (T27.1) mentions removal in v0.5.0.
...
- The operator made the four decisions that were blocking the content work; baked
  ...
  - **T27.7 alias removal DECIDED:** `run`/`propose`/`mix kazi.run` removed in
    **v0.5.0** (next minor); the deprecation hint names v0.5.0.
