...
State of `main`: **803 tests pass** (62 doctests, 741 tests), 18 excluded
(`:nats`/`:graphify` integration tags); `mix format --check-formatted` clean;
`mix compile --warnings-as-errors` clean. E8 T8.1 (profiles+registry, PR #80),
T8.2 (CliAdapter, PR #82), T8.4 (opencode profile, PR #84), and T8.5 (resolution,
PR #83) merged; T8.3/T8.6/T8.8 now unblocked.
...
- **Wave E8-3 (now, fan-out):** T8.3, T8.6, T8.8   (neutral prompt refactor [deps T8.2]; goal-file `[harness]` table [deps T8.5]; local-provider config [deps T8.4] -- independent, parallel).
- **Wave E8-4 (integrate):** T8.7   (Runtime/CLI/authoring/adopt wiring; deps T8.2/T8.4/T8.5/T8.6).
